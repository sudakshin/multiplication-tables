...
function generateQuestionOrFinish() {
	const allQuestionsAnswered = Object.keys(correctAnswerCounts).every(
		(key) => correctAnswerCounts[key] >= 3
	);

	if (allQuestionsAnswered) {
		document.getElementById('question').textContent =
			'Congrats! You have practiced that table.';
		document.getElementById('feedback').textContent = '';
	} else {
		generateQuestion();
	}
...
document
	.getElementById('range-11-20')
	.addEventListener('click', () => setRange(11, 20));
document
	.getElementById('range-2-20')
	.addEventListener('click', () => setRange(2, 20));
document
	.getElementById('range-21-30')
	.addEventListener('click', () => setRange(21, 30));
document
	.getElementById('range-2-30')
	.addEventListener('click', () => setRange(2, 30));
